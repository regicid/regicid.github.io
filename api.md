...
Si vous préférez ne pas vous casser la tête, vous pouvez utiliser les packages R, python et Ruby (voir ci-dessous), qui sont des "wrappers" de cette API.
Nos bases sont en SQLite, et structurées avec les colonnes suivantes : n (nombre d'occurrences, gram (mot ou syntagme recherché), année, mois et jour (selon le corpus). Là où c'est nécessaires, elles sont doublées d'une structure fulltext (fts5), qui permet des interrogations plus complexes. L'API ajoute une colonne "total", qui donne le nombre total de mots ou groupes de mots de cette taille dans le corpus, sur chaque période. On peut donc calculer la fréquence en divisant la colonne "n" par la colonne "total".  Les bases ont été constituées légèrement différement selon le corpus :
